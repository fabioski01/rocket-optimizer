...
V_tank = pi*(phi(end)/2 - th(end))^2*l(end-1);
V_shell = pi*(phi(end)/2)^2*l(end-1) - V_tank; % Volume of the rocket shell for the lenght of a single tank [m^3]
...
M = [V1*rho, V2*rho + M_pl, V3*rho,...
     M_23,...
     V_shell*rho + M_prop(end), V_eng_sl*rho + M_eng_sl*n_e]; % Mass of each component [kg]
