...
% assuming raptor 3 for first stage
% c_eff_raptor3sealevel = 3.21E4; % m/s
% T_raptor3 = 2.64E6; % 2.64MN raptor 3 max T
% % Mass_ratio_first = M0_second/M0_first;
% [altitude_first, tburn_first] = calculate_altitude_extinction(c_eff*1000, T_raptor3, Mass_ratio_stage, M0_first)
...
% Altitude deltah_i or Ze_i (extinction altitude) considering constant
% Thrust burn program, considering zero initial velocity, considering
% homogeneous gravity, ignoring drag
function[Ze, tburn] = calculate_altitude_extinction(c_eff, T, Mass_ratio_section, M0_section)
    g0 = 9.81 % Earth gravit constant m/s^2
    psi = T/(M0_section*g0) % thrust parameter (not constant)
    tburn = c_eff/g0*psi * (1-(1/Mass_ratio_section)) % burn time assuming constant T program
    Ze = (c_eff^2 / psi*g0)*(1- (1/Mass_ratio_section)*(log(Mass_ratio_section)+1)) - 0.5*g0*tburn^2
end
